        ...
        {Object.entries(sessions).map(([venueId, sessions]) => (
          <div key={venueId} className='mb-4'>
            <h3 className='text-lg mb-4'>
              <Link to={`/result/${yearName}/venue/${sessionVenues[venueId].slug}`} className={linkStyles.join(' ')}>
            ...
            <ul className='list-disc list-inside pl-5'>
              {sessions.map((session, index) => (
                <li key={index}>
                  ...
                  {session.contactId && (
                    <span className='ml-2 text-sm text-gray-500 text-base'>Contact: <Link className={linkStyles.join(' ')} to={`result/${yearName}/player/${sessionContacts[session.contactId].slug}`}>{sessionContacts[session.contactId].name}</Link></span>
                  )}
                </li>
              ))}
            </ul>
